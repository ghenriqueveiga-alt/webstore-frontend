...
@Component({
  selector: 'app-grade-compacta',
  imports: [RouterLink, NgClass, KeyValuePipe],
  templateUrl: './grade-compacta.html',
  styleUrl: './grade-compacta.css',
})
export class GradeCompacta implements OnInit, OnDestroy {
  ...
  readonly diasFull = ['Segunda-feira', 'Terça-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira', 'Sábado', 'Domingo'];
  ...
  get filteredBlocos(): BlocoOutput[] {
    const dia = this.diasFull[this.selectedDay()];
    return this.blocos()
  ...
  get faixaCount(): Map<string, number> {
    const map = new Map<string, number>();
    for (const b of this.filteredBlocos) {
      const f = b.aFaixaHorarioDesc ?? 'Outro';
